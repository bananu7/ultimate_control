...
fn write_address_pair<Writer: Write>(ap: AddressPair, w: &mut Writer) {
    w.write(&ap.a.to_le_bytes());
    w.write(&ap.b.to_le_bytes());
}

fn write_packet<Writer: Write>(p: UcPacket, w: &mut Writer) {
    w.write(&[b'U', b'C', 0, 1]);
    ...
    match p {
        ...
        UcPacket::UM(buf) => {
            let size: u16 = (buf.len() as u16).try_into().unwrap();
            w.write(&size.to_le_bytes());
            w.write(&[b'U', b'M']);
            w.write(&buf);
        }
...
fn read_packet(stream: &mut TcpStream) -> std::io::Result<()> {
...
fn parse_packet(bytes: &Vec<u8>) -> std::io::Result<UcPacket> {
    match (bytes[0], bytes[1]) {
        ...
        (b'U',b'M') => {
            let payload: [u8; 6] = bytes[4..10].try_into().expect("Not enough bytes for UM packet");
        ...
        _ => Err(Error::new(ErrorKind::Other, "Ivalid packet"))
    }
}
